...
if (
    ...
    if (isContentEditable()) {
      ...
      onEditableClick()
        ...
        let count = 0
          function refreshBox() {
    setTimeout(() => {
        if(recentConvoDate == undefined){document.getElementById("macro-prompt").style.backgroundColor = "#C72222"}else{document.getElementById("macro-prompt").style.backgroundColor = "#4CAF50"}
      count++
        console.log(recentConvoDate == undefined)
        if(count<50){refreshBox()}//console.log(`it has been ${debugCount} seconds since start of program`);refresh();debugCount+=5
    }, 100)
  }
    } else {
  ...
  function refresh() {
          ...
          if (recentConvoDate != undefined) {recentConvoDate = result
          }
        })
    setTimeout(() => {
...
if (window.location.href.indexOf("/agent/tickets/") != -1){
    ...
    function dateRefresh() {
      ...
      function makeSure(){count=8;dateRefresh()}
  }
  // Initial URL check
