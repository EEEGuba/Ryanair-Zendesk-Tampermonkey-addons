...
if (
    ...
    let customerData = {customerName:'Customer'};
    let dataColumn = undefined
    let dumfix = true
    ...
    function isContentEditable() {
        ...
        }                                           //both of these ^ \/ functions can be capped prolly, i dont check this no mo
        return false
    }
    ...
    function recentConversationDate() {
        ...
        ///////// W I P
        let disruptionColumn;
        dataColumn = Array.from(document.querySelectorAll('[data-test-id="column-1"]'))
        const parentOfColumn = dataColumn.map((column)=>{return column.parentElement})
        parentOfColumn.forEach(element=>{
           const number = element.getAttribute('data-test-id')
              if(number.match(/\d+/)[0]==currentTicketNr&&dumfix){
                  setTimeout(function(){const test = element.querySelector('[data-app-title="Flights Disruptions"]')//this returns null for some reason, should return the damn app
                  console.log(collectTextAndValues(test))},5000)
                      // Store the columnDataArray results
                  const columnDataArray = [];
                  collectTextAndValues(element.children[0]);
                  collectTextAndValues(element.children[2]);
                  //if (!columnDataArray.includes('No disruptions detected.')){setTimeout(function(){collectTextAndValues(element.children[2]);console.log(columnDataArray)},5000)}
                  detailsPicker(columnDataArray)

    // Recursive function to walk through all children
        function collectTextAndValues(element) {
        if (!element) return;
        ...
        // If it's a form field, collect its value
        if (element instanceof HTMLInputElement) {
            if (element.value.trim()) {
                columnDataArray.push(element.value.trim());
            }
        } else {
            // Otherwise collect innerText if it's meaningful
            const text = element.innerText?.trim();
            if (text && text!=columnDataArray[columnDataArray.length-1]&&!text.includes('\n')) {
                columnDataArray.push(text);
            }
        }

        // Recurse into children
        for (const child of element.children) {
            collectTextAndValues(child);
        }
    }
                  function detailsPicker(array){ //PRINTS PNR LESGOO
                      function dateFormatSwitcher(date){const dateArray=date.split(" ");return `${dateArray[1].substring(0,2)}/${months.indexOf(dateArray[0])}/${dateArray[2]}`}
                  if(array.includes('Booking Reference (PNR)*')){customerData.pnr=array[array.indexOf('Booking Reference (PNR)*')+1]}
                  if(array.includes('Flight Number*')){customerData.flightNumber=array[array.indexOf('Flight Number*')+1]}
                  if(array.includes('Departure*')){customerData.departureAirport=array[array.indexOf('Departure*')+1]}
                  if(array.includes('Destination*')){customerData.arrivalAirport=array[array.indexOf('Destination*')+1]}
                  if(array.includes('Travel Date*')){customerData.travelDate=dateFormatSwitcher(array[array.indexOf('Travel Date*')+1])}
        ...
        if (custNameArray.length>1){
            ...
            if(treatedName.length<=2){
               treatedName = custNameArray[1]
              treatedName = String(treatedName[0]).toUpperCase() + String(treatedName).slice(1);
          }
            //treatedName[0].toUpperCase()
            customerData.customerName = treatedName
            }
        else{customerData.customerName='Customer'}
        if (dates.length === 0) {
  ...
  return ord;
    ...
    async function processString(inputString) {
        ...
        let decodedString = inputString.replace(/%5cn/g, "\n").replace(/Customer/g,customerData.customerName).replace(/%27/,"'")

        if (recentConvoDate) {
        // Replace [RECENTDATE] with the actual date

            let finalString = decodedString.replace(/\[RECENTDATE\]/g,recentConvoDate).replace(/\[CUSTOMER\]/g,customerData.customerName).replace(/\[PNR\]/g,customerData.pnr).replace(/\[TRAVELDATE\]/g,customerData.travelDate).replace(/\[DESTINATION\]/g,customerData.arrivalAirport).replace(/\[DEPARTURE\]/g,customerData.departureAirport).replace(/\[FLIGHTNUMBER\]/g,customerData.flightNumber)
            return finalString
